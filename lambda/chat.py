...
def handler(event, context):
    ...
    try:
        ...
        logger.info(
            f"Processing message for session {session_id}: {message}"
        )
    ...
    try:
        response = chat_sessions_table.query(
            KeyConditionExpression=(
                boto3.dynamodb.conditions.Key('session_id').eq(session_id)
            ),
            Limit=limit,
...
def store_message(session_id, message):
    ...
    try:
        ...
        logger.info(
            f"Stored {message['role']} message for session {session_id}"
        )
