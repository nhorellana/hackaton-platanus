  ...
  const categorizeJobs = () => {
    return {
      market: jobs.filter(
        (j) =>
          j.job_type === "research" ||
          j.job_type === "market_research" ||
          j.job_type === "data"
      ),
      internal: jobs.filter((j) => j.job_type === "slack"),
      external: jobs.filter(
        (j) => j.job_type === "external_research" || j.job_type === "mail"
      ),
    };
  };
    ...
    const pollJobs = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/jobs?session_id=${sessionId}`
        );
        if (response.ok) {
  ...
  const getServiceConfig = (type: JobType) => {
    ...
    > = {
      slack: {
        ...
        color:
          "from-pink-500/20 to-purple-500/20 border-pink-500/50 text-pink-400",
      },
      data: {
        name: "Datos Internos",
                ...
                <div className="max-w-md text-center">
                  ...
                  <div className="mb-6 flex justify-center">
                    <div className="relative">
                      {/* Outer spinning ring */}
                      <div className="h-24 w-24 rounded-full border-4 border-(--color-border) border-t-(--color-primary) animate-spin"></div>
                      {/* Inner pulsing circle */}
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="h-16 w-16 rounded-full bg-(--color-primary) opacity-20 animate-pulse"></div>
                      </div>
                    </div>
                  </div>
                  ...
                  <h3 className="text-xl font-semibold text-(--color-text) mb-3">
                    Preparando agentes para investigar tu problema
                    profundamente...
                  </h3>
                  <p className="text-sm text-(--color-text-secondary) mb-6">
                    Nuestros agentes especializados están analizando obstáculos,
                    soluciones, competencia, marco legal y dinámicas de mercado.
                  </p>

                  {/* Progress dots */}
                  <div className="flex justify-center gap-2">
                    <div
                      className="h-2 w-2 rounded-full bg-(--color-primary) animate-bounce"
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1">
                          <div
                            className={`flex h-8 w-8 items-center justify-center rounded-lg border bg-linear-to-br ${config.color}`}
                          >
                            {config.icon}
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <svg
                            className={`h-5 w-5 text-(--color-text-secondary) transition-transform ${
                                ...
                                <div className="space-y-2">
                                  ...
                                  {instructions.contact.justification && (
                                    <div className="text-xs text-(--color-text-secondary) bg-(--color-background) rounded-lg p-2 border border-(--color-border)">
                                      <div className="font-medium text-(--color-text) mb-1">
                                        Contexto
                                      </div>
                                      {instructions.contact.justification}
                                    </div>
                                  ...
                                  {instructions.contact.questions &&
                                    instructions.contact.questions.length >
                                      0 && (
                                      <div className="space-y-2">
                                        ...
                                        {instructions.contact.questions.map(
                                          (question: string, idx: number) => (
                                            <div
                                              key={idx}
                                              className="bg-(--color-background) rounded-lg p-2 border border-(--color-border)"
                                            >
                            ...
                            instructions && (
                              <div className="space-y-3">
                                            ...
                                            {instructions.findings.obstacles.critical_insights
                                              ...
                                              .map(
                                                (
                                                  ...
                                                  <li
                                                    key={idx}
                                                    className="text-xs text-(--color-text) leading-relaxed flex gap-2"
                                                  >
                                            ...
                                            {instructions.findings.solutions.digital_solutions
                                              ...
                                              .map(
                                                (
                                                  ...
                                                  <div
                                                    key={idx}
                                                    className="bg-(--color-background) rounded p-2 border border-(--color-border)"
                                                  >
                                            ...
                                            <div className="grid grid-cols-3 gap-2">
                                              {instructions.findings.market
                                                ...
                                                <div className="bg-(--color-background) rounded p-2 border border-(--color-border)">
                                                  <p className="text-[10px] text-(--color-text-secondary) mb-1">
                                                    TAM
                                                  </p>
                                ...
                                {instructions.queries &&
                                  instructions.queries.length > 0 && (
                                    <div>
                                      <p className="text-[10px] font-medium text-(--color-text-secondary) uppercase tracking-wider mb-2">
                                        Consultas Realizadas (
                                        {instructions.queries.length})
                                      </p>
                                      <div className="space-y-1">
                                        {instructions.queries
                                          ...
                                          .map((query: string, idx: number) => (
                                            <div
                                              key={idx}
                                              className="text-xs text-(--color-text-secondary) bg-(--color-background) rounded p-2 border border-(--color-border) leading-relaxed"
                                            >
                                              • {query}
                                            </div>
                                          ))}
                                          ...
                                          <div className="mt-3 space-y-2">
                                            ...
                                            {job.result.sources
                                              ...
                                              .map((source, idx) => (
                                                <div
                                                  key={idx}
                                                  className="bg-(--color-input-bg) rounded p-2 border border-(--color-border)"
                                                >
